...
def check_fn(streets, intersections, unrounded_intersections):
    for s1 in streets:
        for p in streets[s1]:
            for street in streets:
                if s1 != street:
                    if p in streets[street] and p not in intersections:
                        ...
                        unrounded_intersections.append(p)
    
    return intersections, unrounded_intersections
...
def gg(streets):
            ...
            for i in range(adjust_outer, len(edges)-1):
                ...
                for j in range(i+1,len(edges)):
                    if edges[i][0] in edges[j + adjust_inner] or edges[i][1] in edges[j + adjust_inner]:
                        ...
                        if inter:
                            rounded_inter = [round(x, 2) for x in inter]
                            if rounded_inter not in edges[j + adjust_inner]: 
                                edges.insert(j + adjust_inner + 1,[rounded_inter,edges[j+adjust_inner][1]])
                                edges[j+adjust_inner][1] = rounded_inter
                                adjust_inner += 1
                            if rounded_inter not in temp_intersections:
                                temp_intersections.append(rounded_inter)
                                unrounded_temp_intersections.append(inter)
                if len(temp_intersections) != 0:
                    ...
                    for pt_inter in temp_intersections:
                        if pt_inter not in intersections:
                            index = temp_intersections.index(pt_inter)
                            intersections.append(pt_inter)
                            unrounded_intersections.append(unrounded_temp_intersections[index])
    ...
    intersections, unrounded_intersections = check_fn(streets, intersections, unrounded_intersections)

    ###find vertex and edges
    vertices.extend(intersections)
    unrounded_vertices.extend(unrounded_intersections)
    ...
    for i in range(len(edges)):
        if edges[i-adjust_final][0] in intersections and edges[i-adjust_final][1] in intersections:
            ...
            unrounded_vertices.append(edges[i-adjust_final][1])
            ...
            unrounded_vertices.append(edges[i-adjust_final][0])
    ...
    print("V " + str(len(vertices)),file = sys.stdout, flush = True)

    print("E {",end = "",file = sys.stdout)
    for i in range(len(edges)):
        if i == len(edges) - 1:
            print("<"+str(edges[i][0])+","+str(edges[i][1])+">",end = "",file=sys.stdout)
            break
        print("<"+str(edges[i][0])+","+str(edges[i][1])+">,",end = "",file=sys.stdout)
